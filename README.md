...
`init()` only applies `appcastUrl`, `publicEdKey`, and `httpHeaders` the first time it successfully starts the updater; a later call in the same process is a no-op that returns the existing state without re-applying any of them — so, for example, calling `init()` again to rotate an auth token has no effect until the app restarts.
